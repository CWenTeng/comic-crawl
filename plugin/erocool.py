...
def img_list_parse(html, task):
    ...
    try:
        ...
        parse_log.info(f'erocool   book_title: {book_title}')
        try:
            # 创建漫画书目录
            ...
            parse_log.error(f'erocool   book_title:{book_title}; error:{traceback.format_exc()}')
            raise

        # 图片列表
        imgUrls = html.xpath("//div[@align='center']/noscript/img[@class='vimg']/@src")
        parse_log.info(f"erocool  imgSize:{len(imgUrls)};  imgUrls:{imgUrls}")
        # 封面
        # HomeImg = ''
        imgNum = 1
        # 遍历列表
        for imgUrl in imgUrls:
            try:
                ...
                parse_log.warning(f'erocool   imgUrl:{imgUrl};  imgName:{imgName};  filePath:{book_filePath};   warn: {traceback.format_exc()}')
            imgNum += 1
        # 下载封面
        if imgUrls[0]:
            try:
                ...
                parse_log.warning(f'erocool   HomeImg warn:{traceback.format_exc()}')
    except:
        parse_log.error(f'erocool  error:{traceback.format_exc()}')
