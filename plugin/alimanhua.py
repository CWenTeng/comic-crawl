...
def chapter_list_parse(html, task):
    ...
    try:
        book_title = html.xpath('//div[@class="titleInfo"]/h1/text()')[0]
    except:
    ...
    for li in lis:
        try:
            ...
            if 'http' not in url:
                url = 'http://www.alimanhua.com' + url
...
def img_list_parse(html, task):
    ...
    try:
        ...
        # 取 img 列表
        chapterImages = utils.runJs(ALI_MANHUA_JS['js'], ALI_MANHUA_JS['funcName'],packed)
    ...
    for img in imgList:
        try:
            ...
            if 'http://' not in imgUrl:
                imgUrl = 'http://res.img.jituoli.com/' + imgUrl
            img_num = str(num)
            # 补全集数编号位数
            while img_num:
                if len(img_num) >= 3:
