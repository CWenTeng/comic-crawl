    ...
    for img in imgList:
        try:
            ...
            if chapterPath and (chapterPath not in imgUrl) and ('http' not in imgUrl):
                if host != False:
                    try:
                        jsUrl = "https://www.36mh.com/js/config.js"
                        jsContent = Download().page_down(jsUrl,config.IMG_LIST_HEAD)
                ...
                if host:
                    imgUrl = '/'.join([host, chapterPath, imgUrl])
                else:
                    imgUrl = '/'.join(['https://img001.bolong-aterials.com', chapterPath, imgUrl])
            # 补全集数编号位数
            while img_num:
                if len(img_num) >= 3:
                    break
