...
def chapter_list_parse(html, task):
    ...
    try:
        ...
        if book_title and book_title[0].strip() :
            ...
            book_title = html.xpath("//h1/span/text()")[0]
...
def img_list_parse(html, task):
    ...
    try:
        ...
        raise
    path = config.PATH
    head = {"referer": "https://www.zuimh.com/"}
    # 图片编号
    num = 1
    # 匹配图片列表
    try:
        ...
        parse_log.error(f'{task.site_name}  chapterImages:{chapterImages};  error:{str(traceback.format_exc())}')
        raise
    for img in imgList:
        try:
            # 格式化链接特殊字符
